...
serve(async (req) => {
  ...
  try {
    ...
    if (!betaApplication) {
      ...
      if (pendingApp) {
        if (pendingApp.status === "pending") {
          message =
            "Your beta application is under review. Please wait for approval.";
        } else if (pendingApp.status === "rejected") {
          message =
            "Your beta application was not approved. Please contact support if you believe this is an error.";
        }
      }
    ...
    console.log(
      `Approved beta signup for: ${userEmail} (${betaApplication.name})`,
    );
