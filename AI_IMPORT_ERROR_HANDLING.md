  ...
  async executeWithFallback<T>(
    ...
    for (let i = 0; i < providers.length; i++) {
      ...
      try {
        ...
        console.warn(
          `Provider ${provider.name} failed for ${context.task}:`,
          error.message,
        );
    ...
    throw new Error(
      `All providers failed for ${context.task}. Last error: ${lastError?.message}`,
    );
  }
...
export function getUserFriendlyError(error: ProcessingError): {
  ...
  return {
    ...
    canRetry: ["automatic_retry", "system_retry", "manual_retry"].includes(
      config.action,
    ),
  };
}
