...
export const DEFAULT_WORKFLOW_MODELS: WorkflowModelConfiguration = {
  ...
  externalValidation: {
    ...
    qualitySettings: {
      minConfidence: 0.7, // Lower threshold for validation queries
...
export class ConfigurationResolver {
  static resolve(
    ...
    if (region && REGIONAL_DEFAULTS[region]) {
      resolved = this.mergeRegionalDefaults(
        resolved,
  ...
  private static mergeUserOverrides(base: any, overrides: any): any {
  ...
  private static validateModelConfigs(
    models: WorkflowModelConfiguration,
  ): void {
    for (const [stepName, stepConfig] of Object.entries(models)) {
      if (!stepConfig.primary) {
        throw new Error(
          `Missing primary model configuration for step: ${stepName}`,
        );
      }

      if (!stepConfig.primary.provider || !stepConfig.primary.model) {
        throw new Error(
          `Invalid primary model configuration for step: ${stepName}`,
        );
      }
      ...
      if (stepConfig.primary.timeout && stepConfig.primary.timeout < 1000) {
        console.warn(
          `Very low timeout (${stepConfig.primary.timeout}ms) for step: ${stepName}`,
        );
      }
    }
  }
    ...
    for (const [category, categoryThresholds] of Object.entries(thresholds)) {
      for (const [level, threshold] of Object.entries(categoryThresholds)) {
        if (threshold < 0 || threshold > 1) {
          throw new Error(
            `Invalid confidence threshold for ${category}.${level}: ${threshold}`,
          );
        }
      }
      ...
      if (
        cat.autoReject >= cat.humanReview ||
        cat.humanReview >= cat.autoAccept
      ) {
...
const costOptimizedConfig = ConfigurationResolver.resolve(SYSTEM_DEFAULTS, {
  models: {
  ...
  quality: {
    ...
    confidenceThresholds: {
      autoAccept: {
        generalData: 0.7, // Accept lower confidence for non-critical data
      },
    },
  },
});
