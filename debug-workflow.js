...
function analyzeWorkflowRecording(filePath) {
  ...
  console.log(
    `- Text Length: ${recording.input?.inputs?.text?.length || 0} chars`,
  );
  ...
  recording.steps?.forEach((step, index) => {
    ...
    if (step.stepName === "multi_node_processing") {
      console.log(`   🎯 Multi-Node Results:`, {
        ...
        hasStructuredData: !!(
          outputState.report &&
          typeof outputState.report === "object" &&
          !Array.isArray(outputState.report)
        ),
      });
  ...
  if (finalState.report) {
    console.log("Report Structure:", {
      ...
      keys:
        finalState.report &&
        typeof finalState.report === "object" &&
        !Array.isArray(finalState.report)
  ...
  if (!finalState.report) {
    ...
    console.log(
      "❌ Report is array instead of structured object - check SSE endpoint compilation",
    );
  ...
  if (multiNodeResults) {
    console.log(
      `✅ Multi-node processing executed ${multiNodeResults.processedNodes?.length || 0} nodes`,
    );
...
if (!filePath) {
  console.log(
    "Usage: node debug-workflow.js <path-to-workflow-recording.json>",
  );
  console.log("");
  console.log("Example:");
  console.log(
    "node debug-workflow.js test-data/workflows/workflow-analysis-2025-07-13T11-31-01-543Z.json",
  );
  process.exit(1);
}
