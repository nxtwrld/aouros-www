...
export const GET: RequestHandler = async ({ url, locals: { supabase }, cookies }) => {
  ...
  if (token_hash && type) {
    ...
    if (lastAttempt && (now - lastAttempt) < 5000) { // 5 second window
      console.log('[Auth Confirm] Duplicate confirmation attempt detected, ignoring')
      redirectTo.pathname = '/auth/error'
      redirectTo.searchParams.set('error', 'Duplicate request')
      redirectTo.searchParams.set('details', 'This link was already used recently')
    ...
    if (!error && data?.session) {
      ...
      const postAuthCookies = postVerifyCookies.filter(c => c.name.includes('sb-') || c.name.includes('supabase'))
      ...
      console.log('[Auth Confirm] SUCCESS - Redirecting to:', redirectTo.toString())
      ...
      console.error('[Auth Confirm] Verification failed:', {
        ...
        tokenType: type,
        isExpiredError: error?.message?.includes('expired') || error?.message?.includes('invalid')
      })
    }
  }
  // Handle old flow (code) - Redirect to client-side handler
  else if (code) {
    console.log('[Auth Confirm] Code flow detected, redirecting to client-side handler')
    // Redirect to a client-side route that can handle OAuth codes
    redirect(303, `/auth/confirm-client?code=${encodeURIComponent(code)}&next=${encodeURIComponent(next)}`)
  } 
  else {
  ...
  redirectTo.searchParams.set('details', !token_hash ? 'Missing token' : !type ? 'Missing type' : 'Verification failed')
  redirect(303, redirectTo)
}