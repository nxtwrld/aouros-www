...
export async function POST({ request, params, locals: { supabase, safeGetSession }}) {
    ...
    const { error: errorUploading } = await supabase.storage.from('avatars').upload(params.pid +'_'+filename, file)
    if (errorUploading) {
        throw errorUploading
    }
    console.log('uploaded', filename)

    const { error: errorUpdate } = await supabase.from('profiles').update({ avatarUrl: filename }).match({ id: params.pid })
    if (errorUpdate) {
        console.log('error updating profile', errorUpdate)
        await supabase.storage.from('avatars').remove([params.pid +'_'+filename]);
        throw errorUpdate
    }
