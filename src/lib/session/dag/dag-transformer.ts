...
import type { DAGNode, DAGLink, D3DAGNode, D3DAGLink } from '$components/session/types/dag';
import dagConfig from '$lib/config/dag-default.json';
import { DAG_VISUAL_CONFIG, createForceSimulation } from '$components/session/config/dag-visual-config';
...
export function calculateInitialPositions(
  ...
  const layerWidth = (width - DAG_VISUAL_CONFIG.layout.margins.left - DAG_VISUAL_CONFIG.layout.margins.right) / (maxLayer + 2);
  const usableHeight = height - DAG_VISUAL_CONFIG.layout.margins.top - DAG_VISUAL_CONFIG.layout.margins.bottom;
  
  layerGroups.forEach((layerNodes, layer) => {
    const nodeCount = layerNodes.length;
    const nodeSpacing = Math.min(DAG_VISUAL_CONFIG.layout.nodeSpacing, usableHeight / (nodeCount + 1));
    
    // Sort nodes by category for better grouping
    layerNodes.sort((a, b) => a.category.localeCompare(b.category));
    
    layerNodes.forEach((node, index) => {
      const x = DAG_VISUAL_CONFIG.layout.margins.left + layerWidth * (layer + 1);
      const y = DAG_VISUAL_CONFIG.layout.margins.top + nodeSpacing * (index + 1) + 
...
export function shouldTriggerExpert(
  ...
  for (const symptom of symptoms) {
    ...
    const mappedExperts = dagConfig.triggerConditions.symptomMapping[normalizedSymptom] || [];
...
export function getSubSpecialtyTriggers(
  ...
  const triggers = dagConfig.triggerConditions.subSpecialtyTriggers[parentCategory] || {};
...
export function createExpertNode(
  ...
  const template = dagConfig.expertTemplates[templateKey] || dagConfig.expertTemplates.specialist_expert;
...
export function getAdaptiveForceParams(nodeCount: number) {
  ...
  return {
    ...baseConfig,
    chargeStrength: DAG_VISUAL_CONFIG.forces.chargeStrength * (1 / densityFactor),
    collisionRadius: (node: D3DAGNode) => {
      const baseRadius = typeof DAG_VISUAL_CONFIG.forces.collisionRadius === 'function' 
...
export function groupNodesByCategory(nodes: D3DAGNode[]): Map<string, D3DAGNode[]> {
