...
const SAMPLE_BASED_EXPERT_GENERATION = {
  ...
  hematologist: {
    ...
    customPrompt: "You are a hematologist investigating iron deficiency anemia in a patient with fatigue, heavy menstruation, and hair thinning. Focus on iron studies interpretation and treatment options.",
  ...
  endocrinologist: {
    ...
    customPrompt: "You are an endocrinologist evaluating suspected hypothyroidism in a patient with fatigue, cold intolerance, dry skin, and family history of thyroid disease.",
  ...
  psychiatrist: {
    ...
    customPrompt: "You are a psychiatrist assessing depression in a patient with mood changes, sleep disturbance, and cognitive symptoms. Consider medical causes vs primary psychiatric condition.",
  ...
  gynecologist: {
    ...
    customPrompt: "You are a gynecologist evaluating heavy menstrual bleeding and potential fibroids. Assess need for imaging and treatment options.",
...
export function createRealisticMedicalDAGEvents(sessionId: string): TimedEventStep[] {
  ...
  const triggeredExperts = Object.entries(SAMPLE_BASED_EXPERT_GENERATION)
    .map(([key, expert]) => ({ key, expert }));

  // Step 7: Trigger expert nodes (while quorum manager is still running)
  if (triggeredExperts.length > 0) {
    const expertTriggerEvents: ExpertTriggeredEvent[] = triggeredExperts.map(({ expert }) => ({
  ...
  const sortedExperts = [...triggeredExperts].sort((a, b) => 
...
function generateSpecialistAnalysis(specialistType: string, expert: any) {
  ...
  switch (specialistType) {
    case 'hematologist':
      return {
        ...baseAnalysis,
        recommendation: 'Complete iron studies, consider IV iron therapy if deficient',
        findings: [
    ...
    case 'endocrinologist':
      return {
        ...baseAnalysis,
        recommendation: 'TSH, T4, T3, anti-TPO antibodies; consider thyroid replacement therapy',
        findings: [
    ...
    case 'psychiatrist':
      return {
        ...baseAnalysis,
        recommendation: 'Comprehensive psychiatric evaluation; rule out medical causes first',
        findings: [
    ...
    case 'gynecologist':
      return {
        ...baseAnalysis,
        recommendation: 'Pelvic ultrasound, consider hormonal management options',
        findings: [
...
function generateConsensusAnalysis(triggeredExperts: any[]) {
  ...
  return {
    consensusRecommendation: 'Coordinated multi-specialty evaluation with iron deficiency and thyroid function as primary concerns',
    expertAgreements: [
