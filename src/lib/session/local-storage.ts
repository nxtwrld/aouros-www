...
export class SessionLocalStorage {
  ...
  removeSession(sessionId: string): void {
    ...
    try {
      ...
      log.storage.error(
        "Failed to remove session data from local storage:",
        error,
      );
    }
  }
  ...
  private cleanupOrphanedKeys(): void {
    ...
    try {
      ...
      if (orphanedKeys.length > 0) {
        log.storage.info(
          `Removed ${orphanedKeys.length} orphaned storage keys`,
        );
      }
    } catch (error) {
...
export const saveSessionData = (
  sessionId: string,
  data: Partial<StoredSessionData>,
...
export const clearAllSessionData = () => sessionStorage.clearAllSessions();
