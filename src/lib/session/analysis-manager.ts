...
export class SessionAnalysisManager {
    ...
    answerQuestion(questionId: string, answer: string, confidence?: number): void {
        ...
        if (this.sessionData?.nodes?.actions) {
            this.sessionData.nodes.actions = this.sessionData.nodes.actions.map(action => {
    ...
    handleNodeAction(action: string, targetId: string, reason?: string): void {
        ...
        if (action === 'suppress' && this.sessionData?.nodes?.diagnoses) {
            this.sessionData.nodes.diagnoses = this.sessionData.nodes.diagnoses.map(diagnosis => 
     ...
     */
    static getActionsAffectingDiagnosis(sessionData: SessionAnalysis, diagnosisId: string): {
     ...
     */
    static validateSessionData(sessionData: SessionAnalysis): { isValid: boolean; errors: string[] } {
        const errors: string[] = [];
                ...
                if (action.relationships) {
                    action.relationships.forEach(rel => {
                        if (!allNodes.find(n => n.id === rel.nodeId)) {
                            errors.push(`Action ${action.id} references non-existent node ${rel.nodeId}`);
                        }
                    });
                }
            });
 ...
 */
export function createAnalysisManager(sessionData: SessionAnalysis): SessionAnalysisManager {
