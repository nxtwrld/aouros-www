  ...
  try {
    ...
    const transformed: SessionAnalysis = {
      ...
      nodes: {
        ...
        diagnoses:
          analysisData.currentDiagnosis || analysisData.diagnosis || [],
        treatments:
          analysisData.currentTreatment || analysisData.treatment || [],
...
function handleAnalysisUpdate(event: {
  ...
  const sessionAnalysis = transformAnalysisData(
    event.analysis,
    event.sessionId,
  );
  if (sessionAnalysis) {
  ...
  switch (sseUpdate.type) {
    ...
    case "error":
      ...
      sessionDataActions.setError(
        sseUpdate.data.message || "Unknown error occurred",
      );
      break;

    default:
      logger.session.debug("Unhandled SSE update type", {
        type: sseUpdate.type,
      });
  }
}
...
export function connectSessionToAnalysisStore(sessionId: string): void {
  ...
  logger.session.info("Successfully connected session to analysis store", {
    sessionId,
  });
}
...
export function disconnectSessionFromAnalysisStore(sessionId: string): void {
  logger.session.info("Disconnecting session from analysis store", {
    sessionId,
  });
  ...
  logger.session.info("Successfully disconnected session from analysis store", {
    sessionId,
  });
}
 ...
 */
export async function initializeSessionAnalysis(
  sessionId: string,
  initialData?: SessionAnalysis,
): Promise<void> {
  ...
  try {
    ...
    logger.session.info("Session analysis initialized successfully", {
      sessionId,
    });
  } catch (error) {
 ...
 */
export function getAnalysisConnectionStatus(): {
  isConnected: boolean;
