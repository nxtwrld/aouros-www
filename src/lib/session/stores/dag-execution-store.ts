...
import { createLayoutEngine, DEFAULT_LAYOUT_CONFIG, type DynamicLayoutEngine } from '$lib/session/dag/dynamic-layout-engine';
import type { LayoutNode, LayoutLink, LayoutResult, NodeAdditionOptions } from '$lib/session/dag/dynamic-layout-engine';
...
function createInitialState(): DAGStoreState {
  ...
  if (configuration?.defaultFlow?.nodes) {
    console.log('🔍 Node IDs from configuration:', configuration.defaultFlow.nodes.map((n: any) => n.id));
  }
  ...
  console.log(`✅ DAG Store initialized with ${nodes.size} nodes and ${links.size} links`);
  console.groupEnd();
 ...
 */
function updateStoreFromLayoutResult(state: DAGStoreState, layoutResult: LayoutResult) {
  console.log(`🔄 Updating store from layout result: ${layoutResult.nodes.length} nodes, ${layoutResult.links.length} links`);
  ...
  layoutResult.nodes.forEach((layoutNode: LayoutNode) => {
    ...
    if (existingNode) {
      ...
      console.log(`⬆️ Updated existing node: ${layoutNode.id} at layer ${layoutNode.layer}`);
    } else {
      ...
      console.log(`➕ Created new node: ${layoutNode.id} at layer ${layoutNode.layer}`);
    }
  });
  ...
  layoutResult.links.forEach((layoutLink: LayoutLink) => {
    ...
    if (existing) {
      ...
      console.log(`➕ Added link: ${layoutLink.source} -> ${layoutLink.target}`);
    }
  });
}

function recalculateLayoutInternal(state: DAGStoreState, containerWidth?: number, containerHeight?: number) {
...
export const dagActions = {
  ...
  updateNodeState(nodeId: string, newState: DAGNode['state'], data?: Partial<DAGNode>) {
    dagStore.update(state => {
      ...
      if (node) {
        ...
        console.warn(`⚠️ Node ${nodeId} not found in store when updating state to ${newState}. Available nodes:`, Array.from(state.nodes.keys()));
  ...
  addNodes(nodes: DAGNode[], options?: NodeAdditionOptions) {
    dagStore.update(state => {
      ...
      const layoutNodes: LayoutNode[] = nodes.map(node => ({
        ...
        parallelGroup: node.category === 'ai_generated' ? `${node.parent}_experts` : undefined,
        parentNodes: node.parent ? [node.parent] : [],
        childNodes: node.children || []
      }));
  ...
  addParallelExpert(triggerEvent: ExpertTriggeredEvent) {
    console.log(`🎯 Adding parallel expert: ${triggerEvent.expertName} (${triggerEvent.expertId})`);
