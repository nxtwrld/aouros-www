...
export class ContextAssembler {
  ...
  async assembleContextForAI(
    ...
    try {
      ...
      const contextSections = await this.buildContextSections(matches, query, options);
  ...
  private buildPatientSummary(matches: ContextMatch[]): string {
    ...
    const summaryParts = recentMatches.map(match => 
  ...
  private buildRelevantHistory(matches: ContextMatch[], query: string): string {
    ...
    const historyParts = historicalMatches.map(match =>
  ...
  private buildRecentChanges(matches: ContextMatch[]): string {
    ...
    const changesParts = recentMatches.map(match =>
   ...
   */
  private buildKeyDocuments(matches: ContextMatch[], priorityTypes?: string[]): ContextDocument[] {
    ...
    return relevantMatches.map(match => ({
      ...
      excerpt: match.excerpt || match.metadata.summary.substring(0, 200) + '...',
   ...
   */
  private async buildMedicalContext(matches: ContextMatch[]): Promise<MedicalContext> {
  ...
  private async optimizeContextTokens(
    ...
    const sectionNames = ['summary', 'keyPoints', 'documents', 'medicalContext'];
    const tokenDistribution = TokenOptimizer.calculateTokenDistribution(maxTokens, sectionNames);
    ...
    optimized.keyPoints = TokenOptimizer.optimizeKeyPoints(keyPoints, keyPointsBudget);
    
    // 3. Optimize documents
    const documentsBudget = tokenDistribution.documents;
    const documentExcerpts = sections.keyDocuments.map((d: ContextDocument) => d.excerpt);
    ...
    const keyPointsTokens = optimized.keyPoints.reduce(
      (sum, point) => sum + TokenOptimizer.estimateTokens(`${point.type}: ${point.text}`), 
      0
    );
    ...
    optimized.tokenCount = summaryTokens + keyPointsTokens + documentsTokens + medicalTokens;
  ...
  private calculateContextConfidence(matches: ContextMatch[]): number {
    ...
    const recentnessFactor = matches.filter(m => 
      this.isRecentDocument(m.metadata.date, 180)
