...
export class TokenOptimizer {
  ...
  static optimizeTextArray(
    ...
    let indexedItems = items.map((item, index) => ({ item, index, tokens: itemTokens[index] }));
    ...
    for (const { item, tokens } of indexedItems) {
      if (usedTokens + tokens <= maxTokens) {
        ...
        if (remainingTokens > 50) { // Only if meaningful space left
          ...
          if (truncated.length > 20) { // Only if meaningful content remains
  ...
  static optimizeKeyPoints(
    keyPoints: Array<{ text: string; type: string; confidence: number; date?: string }>,
    maxTokens: number
  ): Array<{ text: string; type: string; confidence: number; date?: string }> {
    if (!keyPoints.length) return [];
    
    // Sort by confidence (higher first)
    const sortedPoints = [...keyPoints].sort((a, b) => b.confidence - a.confidence);
    ...
    for (const point of sortedPoints) {
      ...
      if (usedTokens + pointTokens <= maxTokens) {
        ...
        if (remainingTokens > 20) {
          const truncatedText = this.truncateToTokens(point.text, remainingTokens - 10); // Reserve tokens for type prefix
    ...
    for (const section of sectionData) {
      ...
      if (usedTokens + totalSectionTokens <= maxTokens) {
        ...
        if (remainingTokens > 50) {
          const truncatedContent = this.truncateToTokens(section.content, remainingTokens);
  ...
  static calculateTokenDistribution(
    ...
    const usedTokens = Object.values(distribution).reduce((sum, tokens) => sum + tokens, 0);
