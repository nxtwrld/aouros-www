...
export async function runFullValidation(profileId: string) {
  validationLogger.info('Starting full context assembly validation', { profileId });
  ...
  try {
    ...
    if (!healthCheck.healthy) {
      validationLogger.warn('Health check found issues', { issues: healthCheck.issues });
    ...
    const validation = await validateContextAssemblyIntegration(profileId, testDocument);
    ...
    if (validation.errors.length > 0) {
      validationLogger.error('Validation errors found', { errors: validation.errors });
    }
    
    // Generate recommendations
    const recommendations = generateRecommendations(validation, benchmark, healthCheck);
...
function generateRecommendations(
  ...
  if (!healthCheck.healthy) {
    recommendations.push('Address health check issues before proceeding with production deployment');
  ...
  if (!validation.details.initialization) {
    recommendations.push('Fix context initialization - this is critical for all other features');
  }
  
  if (!validation.details.embeddingGeneration) {
    recommendations.push('Fix embedding generation - required for semantic search functionality');
  }
  
  if (!validation.details.contextAssembly) {
    recommendations.push('Fix context assembly - core functionality for intelligent document retrieval');
  }
  
  if (!validation.details.chatIntegration) {
    recommendations.push('Fix chat integration - required for AI conversations with context');
  }
  
  if (!validation.details.sessionIntegration) {
    recommendations.push('Fix session integration - required for real-time medical consultations');
  }
  
  if (!validation.details.mcpTools) {
    recommendations.push('Fix MCP tools - required for AI to access and analyze documents');
  }
  
  // Performance-based recommendations
  if (validation.performance.totalTime > 5000) {
    recommendations.push('Overall integration is slow (>5s), consider optimizing critical path operations');
  }
  
  if (validation.performance.embeddingTime > 3000) {
    recommendations.push('Embedding generation is slow (>3s), consider switching to a faster model or caching strategy');
  }
...
export async function runLightweightValidation(profileId: string) {
  validationLogger.info('Running lightweight validation for CI/CD', { profileId });
  ...
  try {
    ...
    return {
      ...
      performance: { totalTime: 0, initializationTime: 0, embeddingTime: 0, assemblyTime: 0 }
    };
  }
}