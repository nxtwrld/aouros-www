...
export class ServerChatContextService extends BaseChatContextService {
  ...
  protected async generateQueryEmbedding(query: string): Promise<Float32Array> {
    ...
    logger.namespace('ServerChatContext').info('Using medical terms classification instead of embeddings', {
  ...
  async prepareContextForChat(userMessage: string, options: any = {}): Promise<any> {
    try {
      ...
      logger.namespace('ServerChatContext').info('Extracted medical terms for chat context', {
      ...
      if (searchResult.isError || !searchResult.content) {
        logger.namespace('ServerChatContext').warn('Document search failed or returned no results');
        return {
      ...
      logger.namespace('ServerChatContext').error('Failed to prepare chat context with medical terms', {
