...
export async function runContextValidationTest(
  ...
  try {
    ...
    result.details.profileContext = profileContextManager.isContextReady(profileId);
    ...
    try {
      ...
      result.errors.push(`Chat integration test failed: ${error instanceof Error ? error.message : String(error)}`);
    }

    // Test 4: MCP Tools (basic availability check)
    result.details.mcpTools = typeof medicalExpertTools.searchDocuments === 'function';
    ...
    result.errors.push(`Validation test failed: ${error instanceof Error ? error.message : String(error)}`);
    result.performance.totalTime = performance.now() - startTime;
