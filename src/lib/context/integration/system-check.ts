 ...
 */
async function checkModuleAvailability(modulePath: string, componentName: string): Promise<SystemCheckResult> {
  try {
 ...
 */
function checkServiceMethods(service: any, requiredMethods: string[], componentName: string): SystemCheckResult {
...
export async function performSystemHealthCheck(): Promise<SystemHealthReport> {
  ...
  try {
    ...
    const workflowCheck = checkServiceMethods(
      workflow,
      ['createUnifiedDocumentProcessingWorkflow', 'runUnifiedDocumentProcessingWorkflow'],
  ...
  if (criticalFailures > 0) {
    recommendations.push(`Critical: ${criticalFailures} core components are not available. Context assembly will not work.`);
  }
  
  if (warnings > 0) {
    recommendations.push(`Warning: ${warnings} optional components have issues. Some features may be limited.`);
  }
  ...
  if (overall === 'healthy') {
    recommendations.push('All components are available. Context assembly system is ready.');
  }
...
export async function quickAvailabilityCheck(): Promise<{
  ...
  try {
    ...
    const chatCtx = await import('./chat-service');
    checks.chatIntegration = typeof chatCtx.chatContextService?.prepareContextForChat === 'function';
  } catch {
    // Keep as false
  }
...
export async function generateDeploymentReadiness(): Promise<{
  ...
  if (unavailableRequired.length > 0) {
    blockers.push(`Missing required components: ${unavailableRequired.join(', ')}`);
  }
