...
export class SessionContextService {
  private sessionContextCache = new Map<string, {
  ...
  async initializeSessionContext(
    ...
    try {
      ...
      if (!contextStats) {
        logger.namespace('SessionContext').warn('No context available for session', { 
          sessionId, 
      ...
      if (options.priorityTypes && options.priorityTypes.length > 0) {
        filteredResults = searchResults.filter(result => 
          options.priorityTypes!.some(type => 
      ...
      let contextSummary = 'No relevant medical history found for this session.';
      let tokenUsage = 0;
      ...
      if (filteredResults.length > 0) {
        ...
        contextSummary = this.generateSessionContextSummary(assembledContext, sessionData);
      ...
      logger.namespace('SessionContext').error('Failed to initialize session context', {
        error: error.message,
        sessionId,
        profileId: options.profileId
      });
  ...
  async updateSessionContext(
    ...
    try {
      ...
      if (cached && 
          ...
          Date.now() - cached.lastUpdate < 30000) { // Less than 30 seconds ago
        return cached.result;
      }
      
      // Build context query from recent conversation
      const recentConversation = this.buildConversationContext(sessionData, newTranscripts);
      ...
      const result: SessionContextResult = {
        ...
        relevantHistory: this.extractRelevantHistoryFromContext(chatResult.assembledContext)
      };
      ...
      logger.namespace('SessionContext').error('Failed to update session context', {
        error: error.message,
        sessionId
      });
  ...
  async getContextForAnalysis(
    ...
    try {
      ...
      const relevantHistory = this.filterContextByAnalysisType(context, analysisType);
      ...
      logger.namespace('SessionContext').error('Failed to get context for analysis', {
        error: error.message,
        sessionId,
        analysisType
      });
  ...
  clearSessionContext(sessionId: string): void {
    ...
    logger.namespace('SessionContext').debug('Session context cleared', { sessionId });
  }
  
  /**
   * Build initial context query for session
   */
  private buildSessionContextQuery(sessionData: SessionData, options: SessionContextOptions): string {
    const parts = [
   ...
   */
  private buildConversationContext(sessionData: SessionData, newTranscripts: string[]): string {
    const recentMessages = [
      ...(sessionData.conversationHistory?.slice(-5) || []), // Last 5 messages
      ...newTranscripts.map(text => ({ content: text, role: 'user' as const }))
    ];
   ...
   */
  private generateSessionContextSummary(context: any, sessionData: SessionData): string {
    const parts = [];
    ...
    if (context.medicalContext?.recentChanges?.length) {
      parts.push(`${context.medicalContext.recentChanges.length} recent medical changes`);
    }
    
    if (context.relevantDocuments?.length) {
      parts.push(`${context.relevantDocuments.length} relevant medical documents`);
    }
  ...
  private extractRelevantHistory(searchResults: any[]): string[] {
    return searchResults
      ...
      .map(result => {
        const date = result.metadata.date ? new Date(result.metadata.date).toLocaleDateString() : 'Unknown date';
   ...
   */
  private filterContextByAnalysisType(context: any, analysisType: string): any[] {
    if (!context?.keyPoints) return [];
    ...
    return context.keyPoints.filter((point: any) => 
  ...
  private async generateQueryEmbedding(query: string): Promise<Float32Array> {
    try {
      ...
      logger.namespace('SessionContext').error('Failed to generate query embedding', {
