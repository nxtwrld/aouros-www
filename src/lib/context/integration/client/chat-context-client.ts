/**
 * Client-Side Chat Context Service
 * 
 * Handles chat context integration on the client side using
 * medical terms classification instead of embeddings.
 */

import { BaseChatContextService } from '../shared/chat-context-base';
import { logger } from '$lib/logging/logger';
// Import classification config from a TypeScript module instead of JSON
import { classificationConfig } from '$lib/config/classification';

export class ClientChatContextService extends BaseChatContextService {
  ...
  protected async generateQueryEmbedding(query: string): Promise<Float32Array> {
    ...
    logger.namespace('ClientChatContext').info('Using medical terms classification instead of embeddings', {
   ...
   */
  async prepareContextForChat(userMessage: string, options: any = {}): Promise<any> {
    try {
      // Extract medical terms from the user message
      const queryTerms = this.extractMedicalTermsFromQuery(userMessage);
      
      logger.namespace('ClientChatContext').info('Extracted medical terms for chat context', {
      ...
      if (searchResult.isError || !searchResult.content) {
        logger.namespace('ClientChatContext').warn('Document search failed or returned no results');
        return {
      ...
      const resourceContent = searchResult.content.find(c => c.type === 'resource');
      ...
      return {
        assembledContext: {
          ...
          confidence: Math.max(...documentsData.documents.map((doc: any) => doc.relevance), 0),
          tokenCount: contextSummary.length + documentsData.documents.reduce((sum: number, doc: any) => sum + (doc.summary?.length || 0), 0)
        },
      ...
      logger.namespace('ClientChatContext').error('Failed to prepare chat context with medical terms', {
  ...
  protected extractMedicalTermsFromQuery(query: string): {
    ...
    for (const [, categoryData] of Object.entries(classificationConfig.categories)) {
    ...
    for (const [, temporalData] of Object.entries(classificationConfig.temporalTerms)) {
    ...
    const stopWords = new Set(['the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from', 'up', 'about', 'into', 'through', 'during', 'before', 'after', 'above', 'below', 'between', 'among', 'around', 'under', 'over', 'within', 'without', 'behind', 'beyond', 'beside', 'beneath', 'across', 'against', 'toward', 'towards', 'until', 'unless', 'although', 'because', 'since', 'while', 'where', 'when', 'what', 'which', 'who', 'whom', 'whose', 'why', 'how', 'this', 'that', 'these', 'those']);
...
export const clientChatContextService = new ClientChatContextService();