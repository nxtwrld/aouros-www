...
import { performSystemHealthCheck, quickAvailabilityCheck, generateDeploymentReadiness } from './system-check';
...
export async function runAllSystemChecks() {
  ...
  try {
    ...
    checkLogger.info('Health check results:', {
      ...
      availableComponents: healthReport.components.filter(c => c.available).length,
...
export async function runMinimalCheck() {
  ...
  try {
    const quickCheck = await quickAvailabilityCheck();
    const requiredSystems = ['contextSystem', 'embeddingSystem', 'documentSystem'];
    const criticalFailures = requiredSystems.filter(system => !quickCheck[system as keyof typeof quickCheck]);
