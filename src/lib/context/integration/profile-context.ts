...
export class ProfileContextManager {
  ...
  async initializeProfileContext(
    ...
    if (this.initializationPromises.has(profileId) && !options.forceReinitialize) {
  ...
  private async _performInitialization(
    ...
    try {
      logger.namespace('ProfileContext').info('Initializing profile context with medical terms', {
        profileId,
      ...
      if (!userDocuments || userDocuments.length === 0) {
        logger.namespace('ProfileContext').info('No documents found for profile', { profileId });
  ...
  getContextStats(profileId: string) {
    const documents = get(byUser(profileId));
    const documentsWithTerms = documents.filter(doc => doc.medicalTerms && doc.medicalTerms.length > 0);
  ...
  async refreshContext(profileId: string): Promise<void> {
    return this.initializeProfileContext(profileId, { forceReinitialize: true });
  }
  ...
  cleanupContext(profileId: string): void {
    this.initializationPromises.delete(profileId);
    logger.namespace('ProfileContext').info('Cleaned up context for profile', { profileId });
  }

  /**
   * Add document to context (simplified version for medical terms system)
   */
  async addDocumentToContext(profileId: string, document: Document): Promise<void> {
   ...
   */
  async removeDocumentFromContext(profileId: string, documentId: string): Promise<void> {
