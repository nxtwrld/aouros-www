  ...
  async execute(params: any, profileId: string): Promise<MCPToolResult> {
    try {
      ...
      const profileSummary: ProfileSummary = {
        ...
        conditions: profile.health?.chronicConditions?.map(condition => ({
          name: condition,
          status: "active",
        })) || [],
        medications: profile.health?.currentMedications?.map(med => ({
          name: med,
          status: "active" as const,
        })) || [],
        allergies: profile.health?.allergies?.map(allergy => ({
          substance: allergy,
        })) || [],
