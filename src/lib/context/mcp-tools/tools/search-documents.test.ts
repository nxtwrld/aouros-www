...
vi.mock('../base/base-tool', () => ({
  BaseMedicalTool: class {
    ...
    protected classifyDocumentByDate(docDate: Date, allDocuments: any[]): string {
      const now = new Date();
...
const mockDocuments: (Document | any)[] = [
  {
    ...
    medicalTerms: ['blood', 'glucose', 'cholesterol', 'hdl', 'ldl', 'triglycerides'],
  ...
  },
  {
    ...
    medicalTerms: ['heart', 'medication', 'prescription', 'cardiac', 'lisinopril'],
...
describe('SearchDocumentsTool', () => {
  ...
  describe('Tool Definition', () => {
    ...
    it('should include category and temporal descriptions', () => {
      const definition = searchTool.getToolDefinition();
      const termsDescription = definition.inputSchema.properties.terms.description;
  ...
  describe('Stage 3: Temporal Processing', () => {
    ...
    it('should handle multiple temporal terms', async () => {
      ...
      const text = result.content[0]?.text || '';
      expect(text.includes('temporal:latest') || text.includes('temporal:recent')).toBe(true);
    });
  ...
  describe('Result Formatting', () => {
    ...
    it('should handle documents without titles gracefully', async () => {
      ...
      const mockGetUserDocs = vi.fn().mockResolvedValueOnce(mockDocsWithoutTitle);
  ...
  describe('Performance and Edge Cases', () => {
    ...
    it('should handle documents with missing metadata', async () => {
      ...
      const mockGetUserDocs = vi.fn().mockResolvedValueOnce(mockDocsWithMissingData);
      (searchTool as any).getUserDocuments = mockGetUserDocs;
  ...
  describe('Logging and Debugging', () => {
    it('should log search stages in console', async () => {
      ...
      expect(consoleGroupSpy).toHaveBeenCalledWith('🔍 Three-Stage Medical Document Search');
      expect(consoleLogSpy).toHaveBeenCalledWith('AI-provided search terms:', ['blood']);
