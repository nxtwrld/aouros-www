...
import { BaseMedicalTool, type MCPTool, type MCPToolResult } from '../base/base-tool';
...
export class SearchDocumentsTool extends BaseMedicalTool {
  ...
  async execute(params: any, profileId: string): Promise<MCPToolResult> {
    ...
    console.log(`📊 Documents Loading Debug: Total documents loaded for user ${profileId}: ${allDocuments.length}`);
    ...
    searchResults.forEach((result, index) => {
      ...
      const title = typeof doc.content === 'object' && doc.content?.title 
        ? doc.content.title 
        : `Document ${doc.id}`;
