  ...
  classificationConfig: {
    categories: {
      ...
      IMAGING: {
        ...
        priority: 2,
        keywords: ["x-ray", "mri", "ct", "ultrasound", "scan", "imaging", "radiology"]
      },
      MEDICATIONS: {
        id: "medications",
        name: "Medications & Prescriptions",
        priority: 3,
        keywords: ["medication", "medicine", "prescription", "drug", "treatment", "therapy"]
      }
    },
    temporalTerms: {
      latest: {
...
const integrationTestDocuments: Document[] = [
  {
    ...
    content: {
      ...
      summary: 'Blood test results showing normal white blood cell count, slightly elevated glucose levels at 110 mg/dL, cholesterol within normal range at 180 mg/dL.',
    ...
    medicalTerms: [
      'blood', 'cbc', 'complete blood count', 'glucose', 'cholesterol', 
      'hdl', 'ldl', 'triglycerides', 'wbc', 'white blood cell', 'rbc', 'red blood cell'
    ],
  ...
  },
  {
    ...
    content: {
      title: 'Chest X-Ray',
      summary: 'Chest X-ray shows clear lungs with no signs of pneumonia or other abnormalities. Heart size appears normal.',
    ...
    medicalTerms: [
      'chest', 'x-ray', 'lungs', 'heart', 'radiology', 'pneumonia', 
      'pleural effusion', 'pneumothorax', 'mediastinum', 'bilateral'
    ],
  ...
  },
  {
    ...
    content: {
      ...
      summary: 'Prescription for Lisinopril 10mg daily for blood pressure management and cardiovascular health.',
    ...
    metadata: {
      ...
      tags: ['heart', 'medication', 'prescription', 'blood pressure', 'lisinopril']
    },
    medicalTerms: [
      'heart', 'cardiac', 'medication', 'prescription', 'lisinopril',
      'blood pressure', 'hypertension', 'cardiovascular', 'tablet'
    ],
...
describe('Search Documents Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(searchTool, 'getUserDocuments' as any).mockResolvedValue(integrationTestDocuments);
    vi.spyOn(searchTool, 'getCurrentUser' as any).mockResolvedValue({ id: 'test-user' });
  });
  ...
  describe('MCP Tools Integration', () => {
    ...
    it('should handle MCP tool error responses', async () => {
      ...
      vi.spyOn(searchTool, 'getUserDocuments' as any).mockRejectedValue(new Error('Database error'));
  ...
  describe('Performance and Scalability Integration', () => {
    it('should handle large document sets efficiently', async () => {
      ...
      vi.spyOn(searchTool, 'getUserDocuments' as any).mockResolvedValue(largeDocumentSet);
  ...
  describe('Error Handling and Edge Cases Integration', () => {
    ...
    it('should handle invalid profile IDs', async () => {
      vi.spyOn(searchTool, 'getUserDocuments' as any).mockRejectedValue(new Error('Profile not found'));
    ...
    it('should handle documents with missing or malformed medical terms', async () => {
      ...
      vi.spyOn(searchTool, 'getUserDocuments' as any).mockResolvedValue(malformedDocuments);
  ...
  describe('Real-world Usage Scenarios', () => {
    ...
    it('should handle patient asking about heart medications', async () => {
      ...
      expect(result.content[0].text).toContain('Found');
      expect(result.content[0].text).toContain('Cardiac Medication');
      expect(result.content[0].text).toContain('heart, medication, prescription');
    });
