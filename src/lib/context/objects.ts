...
const anatomy: string[] = Object.keys(objects).reduce((acc, key) => {
    return [...acc, ...objects[key].objects];
}, [] as string[]).map(obj => obj.toLowerCase());
...
export function findObjects (text: string, type: string | undefined = undefined) {
    const lText = text.toLowerCase().replace(/ /ig, '_');
