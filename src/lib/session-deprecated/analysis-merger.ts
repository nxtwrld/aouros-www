  ...
  const keyContent =
    type === "diagnosis"
      ...
      : type === "treatment"
        ...
        : type === "medication"
          ? `${item.name}_${item.dosage}`.toLowerCase()
          : type === "followUp"
            ? item.name?.toLowerCase()
...
function areItemsSimilar(item1: any, item2: any, type: string): boolean {
  ...
  const getKeyText = (item: any) => {
    switch (type) {
      ...
      case "medication":
        return `${item.name} ${item.dosage}`.toLowerCase();
      case "followUp":
        return item.name?.toLowerCase() || "";
    ...
    for (const newItem of newItems || []) {
      ...
      if (existingItems.has(newId)) {
        ...
        merged = {
          ...
          confidence:
            newItem.probability || newItem.confidence || existing.confidence,
          isNew: false,
          isUpdated: true,
  ...
  getStats() {
    ...
    for (const [type, items] of this.mergedItems) {
      ...
      stats[type] = {
        ...
        avgConfidence:
          itemArray.reduce((sum, item) => sum + (item.confidence || 0), 0) /
            itemArray.length || 0,
      };
    }
    return stats;
