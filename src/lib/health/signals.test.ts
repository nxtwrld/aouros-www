...
describe('Enhanced Health Signals Processing', () => {
  ...
  describe('processHealthData function', () => {
    ...
    it('should handle new extracted document format', async () => {
      const { processHealthData } = await import('./signals');
      const { insertMetaHistoryEntries } = await import('./meta-history-storage');
    ...
    it('should create medication effectiveness entries', async () => {
      ...
      await processHealthData(medicationWithEffectiveness, mockPatientId, 'test-doc-3');
      ...
      expect(entryTypes).toContain(MetaHistoryEntryType.MEDICATION_EFFECTIVENESS);
    });
    ...
    it('should create adverse reaction entries', async () => {
      ...
      await processHealthData(medicationWithSideEffects, mockPatientId, 'test-doc-4');
  ...
  describe('Legacy compatibility', () => {
    it('should maintain backward compatibility with updateSignals', async () => {
      ...
      await expect(updateSignals(legacySignals, mockPatientId, 'legacy-doc')).resolves.not.toThrow();
    ...
    it('should handle empty data gracefully', async () => {
      ...
      await expect(processHealthData([], mockPatientId, 'empty-doc')).resolves.not.toThrow();
      await expect(processHealthData({}, mockPatientId, 'empty-obj-doc')).resolves.not.toThrow();
    });
  });
  ...
  describe('Error handling', () => {
    it('should handle profile fetch errors', async () => {
      ...
      await expect(processHealthData([], mockPatientId, 'error-doc')).rejects.toThrow('Profile not found');
    });
    ...
    it('should handle document update errors', async () => {
      ...
      await expect(processHealthData([{
        signal: 'test',
        value: '1',
        date: '2024-01-15T10:00:00Z'
      }], mockPatientId, 'error-doc')).rejects.toThrow('Update failed');
    });
    ...
    it('should handle invalid data formats gracefully', async () => {
      ...
      await expect(processHealthData(invalidData, mockPatientId, 'invalid-doc')).resolves.not.toThrow();
    });
  });
  ...
  describe('Data transformation', () => {
    it('should correctly transform medication data', async () => {
      ...
      const medicationEntry = entries.find((e: any) => e.entryType === MetaHistoryEntryType.MEDICATION_CURRENT);
    ...
    it('should correctly transform lab result data', async () => {
      ...
      const entries = vi.mocked(insertMetaHistoryEntries).mock.calls[0][0];
      const labEntry = entries.find((e: any) => e.entryType === MetaHistoryEntryType.MEASUREMENT_LAB);
