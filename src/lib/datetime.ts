...
export let dayNamesFull = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
export let dayNamesShort = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
export let dayNamesShort2 = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
export let monthNamesFull = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
export let monthNamesShort = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
...
export const today = readable(new Date(), (set) => {
  let incrementCounter = setInterval( () => {
    let newVal = new Date();
    if (newVal.getDate() != oldToday.getDate() || newVal.getMonth() != oldToday.getMonth() || newVal.getFullYear() != oldToday.getFullYear()) {
      set(newVal);
      oldToday = newVal;
    }
  }, 1000*60);
...
export function date(date: string | Date | number | undefined, format: string | undefined = undefined): string | undefined  {
    if (date == undefined) {
        return date;
    }
...
export function time(date: string | Date | number | undefined, format: string | undefined = undefined): string | undefined  {
    if (date == undefined) {
        return date;
    }
...
export function dateTime(date: string | Date | number | undefined, format: string | undefined = undefined) : string | undefined {
    if (date == undefined) {
        return date;
    }
...
export function duration(durationInput: string | undefined) : string | undefined {
...
export function fromDicomDate(date: string, time?: string): Date {
    ...
    if (time) {
        ...
        timeParsed = time1.substring(0,2) + ':' + time1.substring(2,4)+ ':' + time1.substring(4,6);
        if (time2) {
