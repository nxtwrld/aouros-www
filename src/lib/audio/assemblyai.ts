...
export async function transcribeAudio(audioData: File, instructions: { lang: string }) {
    ...
    const audioUrl = await uploadAudio(new Uint8Array(await audioData.arrayBuffer()));
    ...
    const conversation = transcript.words?.reduce((acc, word) => {
