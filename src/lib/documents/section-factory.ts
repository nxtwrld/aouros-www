...
const STANDARD_SECTIONS = {
  ...
  signals: {
    ...
    propsMapper: (doc: Document) => ({ data: doc.content.signals, document: doc })
  },
  text: {
    component: SectionText,
    priority: 40,
    condition: (doc: Document) => !!(doc.content.content || doc.content.localizedContent),
  ...
  attachments: {
    ...
    propsMapper: (doc: Document) => ({ data: doc.content.attachments, key: doc.key })
  }
} as const;
...
const DOCUMENT_TYPE_SECTIONS = {
  ...
  laboratory: {
    ...
    optionalSections: ['diagnosis', 'text', 'performer', 'attachments', 'links'],
...
export function createDocumentSectionPlan(
  ...
  const typeConfig = DOCUMENT_TYPE_SECTIONS[documentType.primaryType as keyof typeof DOCUMENT_TYPE_SECTIONS] || 
 ...
 */
function getSectionWrapper(sectionId: string, documentType: string): SectionConfiguration['wrapper'] {
  ...
  const docTypeWrappers = typeWrappers[documentType as keyof typeof typeWrappers];
  return docTypeWrappers?.[sectionId as keyof typeof docTypeWrappers] || undefined;
}
...
export function validateSectionAvailability(
  ...
  for (const requiredSection of typeConfig.requiredSections) {
    const sectionDef = STANDARD_SECTIONS[requiredSection as keyof typeof STANDARD_SECTIONS];
  ...
  for (const prioritySection of typeConfig.prioritySections) {
    const sectionDef = STANDARD_SECTIONS[prioritySection as keyof typeof STANDARD_SECTIONS];
    if (sectionDef && !sectionDef.condition(document)) {
      warnings.push(`Priority section '${prioritySection}' has no content available`);
    }
  }
