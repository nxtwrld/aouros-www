...
export function detectDocumentType(document: Document): DocumentTypeInfo {
  ...
  if (enhancedType && enhancedType in DOCUMENT_TYPE_MAPPINGS) {
    const mapping = DOCUMENT_TYPE_MAPPINGS[enhancedType as keyof typeof DOCUMENT_TYPE_MAPPINGS];
    return {
  ...
  const content = `${document.content.title} ${document.content.summary || ''} ${document.content.content || ''}`.toLowerCase();
...
export function shouldUseSpecializedViewer(
  ...
  return featureFlags.ENABLE_SPECIALIZED_UI && 
 ...
 */
export function getViewerComponentName(documentType: DocumentTypeInfo): string | null {
