...
export function normalizeName(name: string, options: {
    ...
    // Remove diacritics
    if (opt.removeDiacritics) name = name.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    ...
    reports.forEach((report) => {
        if (report.content?.patient?.fullName) {
            ...
            if (patientMap.has(name)) {
                ...
                if (newProfile.insurance && newProfile.insurance.provider && !profile.insurance.provider) {
                    profile.insurance.provider = newProfile.insurance.provider;
                }
                if (newProfile.insurance && newProfile.insurance.number && !profile.insurance.number) {
                ...
                let profile = Object.assign(getEmptyProfile(), {
                    ...
                    fullName: capitalizeFirstLetters(removePrefixes(report.content.patient.fullName)),
                ...
                if (report.content.patient.insurance) {
                    if (report.content.patient.insurance.number) {
                        profile.insurance.number = removeNonNumeric(report.content.patient.insurance.number)
                    }
                    
                    if (report.content.patient.insurance.provider) {
                        profile.insurance.provider = capitalizeFirstLetters(report.content.patient.insurance.provider);
                    }
                }
                if (report.content.patient.identifier) {
                    let insurance_number = removeNonNumeric(report.content.patient.identifier);
...
export function findInProfiles(contact: {
    ...
    const profilesFound = profilesArray.map(p => {
        ...
        if (names.length > 0) {
            if (names.some(n => searchOptimize(normalizeName(p.fullName)).includes(n))) {
                r.matchName = true;
            }
        }
...
export function normalizePatientData(profile: DetectedProfileData): Profile {
    ...
    if (profile.insurance) {
        ...
        if (profile.insurance.provider) {
            result.insurance.provider = capitalizeFirstLetters(profile.insurance.provider);
        }
    }
