...
export async function encrypt(aesKey: CryptoKey, message: string): Promise<string> {
...
export async function decrypt(aesKey: CryptoKey, encryptedData: string): Promise<string> {
    // Decode the Base64-encoded data
    const combinedData = new Uint8Array(atob(encryptedData).split('').map(char => char.charCodeAt(0)));
