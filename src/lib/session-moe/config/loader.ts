...
class MoEConfigLoader {
  ...
  async loadConfig(customPath?: string): Promise<MoEConfig> {
    ...
    try {
      ...
      logger.moe?.error("Failed to load MoE configuration", {
        error,
        path: configPath,
      });
  ...
  private async loadExpertSets(): Promise<void> {
    try {
      ...
      const expertSetFiles = files.filter(
        (file) =>
      ...
      for (const file of expertSetFiles) {
        ...
        try {
          ...
          logger.moe?.warn(`Failed to load expert set file: ${file}`, {
            error,
          });
        }
      }
      ...
      logger.moe?.info(
        `Loaded ${Object.keys(this.expertSets).length} expert sets`,
      );
      ...
      if (Object.keys(this.expertSets).length === 0) {
        this.expertSets["gp_basic"] = {
          ...
          description:
            "Essential medical analysis for general practice settings",
        ...
        logger.moe?.warn(
          "No expert set files found, created default expert set",
        );
      }
    } catch (error) {
  ...
  private validateConfig(config: MoEConfig): void {
    ...
    if (!this.expertSets[config.defaultExpertSet]) {
      throw new Error(
        `Default expert set '${config.defaultExpertSet}' not found`,
      );
    }
    ...
    Object.entries(config.experts).forEach(([key, expert]) => {
      ...
      if (!provider || !provider.enabled) {
        throw new Error(
          `Expert ${key} references disabled or missing provider ${expert.provider}`,
        );
      }
      ...
      if (!provider.models[expert.modelType]) {
        throw new Error(
          `Expert ${key} references missing model type ${expert.modelType}`,
        );
      }
    });
    ...
    Object.entries(this.expertSets).forEach(([setKey, expertSet]) => {
      ...
      expertSet.experts.forEach((expertKey) => {
        if (!config.experts[expertKey]) {
          throw new Error(
            `Expert set '${setKey}' references non-existent expert '${expertKey}'`,
          );
        }
      });
      ...
      Object.keys(expertSet.consensus.expertWeights).forEach((expertKey) => {
        if (!expertSet.experts.includes(expertKey)) {
          throw new Error(
            `Expert set '${setKey}' has consensus weight for expert '${expertKey}' not in set`,
          );
        }
      });
    });
  ...
  private applyEnvironmentOverrides(config: MoEConfig): void {
    ...
    if (process.env.MOE_PARALLEL_EXECUTION !== undefined) {
      config.performance.parallelExecution =
        process.env.MOE_PARALLEL_EXECUTION === "true";
    }

    if (process.env.MOE_EXPERT_TIMEOUT !== undefined) {
      config.performance.expertTimeout = parseInt(
        process.env.MOE_EXPERT_TIMEOUT,
        10,
      );
    }
  ...
  async getExpertSchema(expertKey: string): Promise<any> {
    ...
    try {
      ...
      const schemaModule = await import(
        `$lib/configurations/moe-experts/${expertConfig.schema.configPath}`
      );

      // Support for schema variants (enhanced, simple, fast, etc.)
      if (
        expertConfig.schema.variant &&
        schemaModule[expertConfig.schema.variant]
      ) {
      ...
      logger.moe?.error(
        `Failed to load expert schema: ${expertConfig.schema.configPath}`,
        { error },
      );
      throw new Error(
        `Failed to load schema for expert ${expertKey}: ${error.message}`,
      );
    }
  }
...
export function getExpertSetConfig(
  expertSetKey: string,
): ExpertSetConfig | undefined {
...
export function getExpertSetConsensus(
  expertSetKey: string,
