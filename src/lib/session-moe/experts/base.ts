...
export abstract class MedicalExpertBase {
  ...
  constructor(protected readonly expertKey: string) {
    try {
      ...
      if (!modelConfig) {
        throw new Error(
          `Model configuration not found for provider: ${expertConfig.provider}, type: ${expertConfig.modelType}`,
        );
      }
  ...
  private initializeLLM(): BaseChatModel {
    ...
    if (!apiKey) {
      throw new Error(
        `API key not found for ${provider}. Set ${providerConfig.apiKeyEnv} environment variable.`,
      );
    }
  ...
  protected buildPrompt(template: string, context: ExpertContext): string {
    return template
      ...
      .replace(
        "{previousAnalyses}",
        JSON.stringify(context.previousAnalyses || []),
      );
  }
...
LANGUAGE: ${context.language}`;
    ...
    if (
      ...
      Object.keys(context.patientHistory).length > 0
    ) {
      content += `\n\nMEDICAL HISTORY:
...
${JSON.stringify(context.previousAnalyses, null, 2)}`;
  ...
  protected async invokeWithSchema(context: ExpertContext): Promise<any> {
    try {
      ...
      logger.moe?.error(
        `Schema-based LLM invocation failed for expert ${this.id}`,
        { error },
      );
      throw error;
    }
...
Please provide your analysis following the structured schema format, ensuring all responses are in ${context.language} language.`;
  ...
  protected calculateConfidence(
    ...
    const weighted =
      baseConfidence * 0.5 + evidenceStrength * 0.3 + consensusSupport * 0.2;
