...
export default class Vaccination implements IAnimation {
    ...
    private velocity: number = .03;
    private group: THREE.Group = new THREE.Group();
    private objectCount: number = 100;

    constructor(scene: THREE.Scene, boundaries?: any) {
        this.scene = scene;
        this.objectBounced = this.scene.getObjectByName('shade_skin').getObjectByName('body')
    ...
    async add() {
        ...
        const objs: THREE.Mesh[] = await Promise.all(models.map(m => {
            return new Promise((resolve, reject) => {
                loader.load('/models/biology/'+m,
                (object: THREE.Mesh) => {
        ...
        for (let i = 0; i < this.objectCount; i++) {
            ...
            object.material = new THREE.MeshStandardMaterial({ color: colors[randomNumber]}); // Red color for example
    ...
    update() {
        const bouncedBoundingBox = new THREE.Box3().setFromObject(this.objectBounced);
        const attractionStrength = 0.05;

        const targetBoundingBox = new THREE.Box3().setFromObject(this.objectBounced);

        this.objects.forEach((object: THREE.Mesh) => {
            // Calculate attraction force towards the cube
            const attractionForce = this.calculateAttractionForce(object, this.objectBounced, attractionStrength);
            ...
            // Boundary checks
            if (object.position.x < this.sceneBoundaries.minX || object.position.x > this.sceneBoundaries.maxX ||
                object.position.y < this.sceneBoundaries.minY || object.position.y > this.sceneBoundaries.maxY ||
                object.position.z < this.sceneBoundaries.minZ || object.position.z > this.sceneBoundaries.maxZ) {
    ...
    randomVelocity(maxSpeed: number, object: THREE.Mesh) {
        ...
        if (Math.random() > 0.7)  {
            ...
            const direction = new THREE.Vector3().subVectors(this.objectBounced.position, object.position);
...
/*
    ...
    isOverlapping(newObject: THREE.Mesh, existingObject: THREE.Mesh) {
        ...
        const existingObjectBoundingBox = new THREE.Box3().setFromObject(existingObject);
    ...
    calculateAttractionForce(object: THREE.Mesh, target: THREE.Mesh, strength: number) {
        const direction = new THREE.Vector3().subVectors(target.position, object.position);
