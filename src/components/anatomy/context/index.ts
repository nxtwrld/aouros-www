...
const anatomyObjects: { [key: string]: string } = Object.keys(objects).reduce(
  (acc, key) => {
...
export function createContextFromTags(
  tags: string[],
  objects?: any[],
): IContext {
  const layers: string[] = [];
  ...
  tags.forEach((tag) => {
    tag = tag.toLowerCase().replace(/ /gi, "_");
