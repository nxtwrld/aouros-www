...
export function constrainTransform(
  ...
  return transform.k === 1 &&
    Math.abs(constrainedX) < 10 &&
    Math.abs(constrainedY) < 10
  ...
  return d3
    ...
    .wheelDelta((event) => {
      ...
      if (event.ctrlKey) {
        ...
        (event as any).wheelDeltaY &&
        Math.abs((event as any).wheelDeltaY) % 3 === 0 &&
        Math.abs((event as any).wheelDeltaY) !== 120
      ) {
...
export function zoomIn(
  ...
  if (!svg || !zoom) return;
  svg.transition().duration(config.duration).call(zoom.scaleBy, 1.5);
}
...
export function zoomToFit(
  ...
  svg.transition().duration(config.duration).call(zoom.transform, transform);
}
