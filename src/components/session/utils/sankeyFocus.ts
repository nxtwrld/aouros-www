...
export function buildFocusableNodesList(sankeyData: any): SankeyNode[] {
  ...
  const orderedNodes: SankeyNode[] = [
    ...
    ...sankeyData.nodes.filter(
      (n: any) => !["symptom", "diagnosis", "treatment"].includes(n.type),
    ),
  ];
...
export function updateNodeFocus(
  ...
  svg
    .select(".node-group")
    .selectAll(".node > .node-html")
    .classed("focused", false);

  if (targetFocusedNodeId) {
    const focusedIndex = focusableNodes.findIndex(
      (node) => node.id === targetFocusedNodeId,
    );
    svg
      .select(".node-group")
      .selectAll(`.node > .node-html[data-node-id="${targetFocusedNodeId}"]`)
      .classed("focused", true);
    onfocusChange?.(
      new CustomEvent("focusChange", { detail: { index: focusedIndex } }),
    );
    return focusedIndex;
  }
...
export function focusNextNode(
  ...
  onfocusChange?.(
    new CustomEvent("focusChange", { detail: { index: nextIndex } }),
  );
  return nextIndex;
}
...
export function focusPreviousNode(
  ...
  onfocusChange?.(
    new CustomEvent("focusChange", { detail: { index: prevIndex } }),
  );
  return prevIndex;
}
 ...
 */
export function isLinkActiveInFocusMode(link: any, activePath: any): boolean {
 ...
 */
export function getConnectedLinkIds(
  nodeId: string,
  sankeyData?: any,
): Set<string> {
...
export function resetHighlighting(
  ...
  classNames.forEach((className) => {
    svg
      .selectAll(`.node-html.${className}, .link.${className}`)
      .classed(className, false);
  });

  // Apply default state classes with transition for smooth animation
  svg.selectAll(".node-html, .link").classed(`state-reset`, true);
}
...
export function updateSelectionState(
  ...
  const highlightedNodeSet =
    hoverNodes.length > 0 ? new Set(hoverNodes) : new Set(activeNodes);
  const highlightedLinkSet =
    hoverLinks.length > 0 ? new Set(hoverLinks) : new Set(activeLinks);
  ...
  nodeHtmlSelection
    ...
    .classed(
      "dimmed",
      (d: any) => hasHighlighted && !highlightedNodeSet.has(d.id),
    )
    .classed(
      "background-trigger",
      (d: any) => hasHighlighted && !highlightedNodeSet.has(d.id),
...
export function resetToDefault(
  ...
  svg
    ...
    .selectAll(".node > .node-html")
    .classed(
      "highlighted dimmed background-trigger background-path active-path selected focused hovered connected-to-selected",
      false,
    )
  ...
  svg
    .selectAll(".link")
    .classed(
      "active-path background-trigger background-path inactive hovered connected-to-selected",
      false,
    );
}
