...
export interface SymptomNodeV2 {
    ...
    source: 'transcript' | 'medical_history' | 'family_history' | 'social_history' | 'medication_history' | 'suspected';
    quote?: string;
...
export interface TreatmentNodeV2 {
    id: string;
    type: 'medication' | 'procedure' | 'therapy' | 'lifestyle' | 'investigation' | 'immediate' | 'referral' | 'supportive';
    name: string;
    dosage?: string;
    priority: number; // 1-10
    confidence: number; // 0-1
...
export interface ActionNodeV2 {
    ...
    category: 'symptom_exploration' | 'diagnostic_clarification' | 'treatment_selection' | 'risk_assessment' |
              'drug_interaction' | 'contraindication' | 'allergy' | 'warning' | 'red_flag';
...
export interface RelationshipV2 {
    nodeId: string;
    relationship: 'supports' | 'contradicts' | 'confirms' | 'rules_out' | 'suggests' | 'treats' |
                  'manages' | 'prevents' | 'relieves' | 'investigates' | 'clarifies' | 'explores' |
                  'excludes' | 'reveals' | 'indicates' | 'requires' | 'monitors';
...
export interface UserActionV2 {
    timestamp: string;
    action: 'suppress' | 'accept' | 'modify' | 'add_note' | 'highlight' | 'question';
    targetId: string;
    reason?: string;
    confidence?: number;
    note?: string;
