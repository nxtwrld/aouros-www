...
import type { DAGVisualizationConfig, D3DAGNode, D3DAGLink } from '../types/dag';
...
export function getNodeStyle(node: D3DAGNode) {
  const stateStyle = DAG_VISUAL_CONFIG.styles.nodes[node.state] || DAG_VISUAL_CONFIG.styles.nodes.pending;
  const categoryColor = NODE_CATEGORY_COLORS[node.category] || '#6B7280';
  
  return {
    ...stateStyle,
    fill: node.state === 'pending' ? stateStyle.fill : adjustColorBrightness(categoryColor, 20),
...
export function getLinkStyle(link: D3DAGLink) {
  ...
  return DAG_VISUAL_CONFIG.styles.links[link.type] || DAG_VISUAL_CONFIG.styles.links.inactive;
}
...
export function calculateFixedPosition(node: D3DAGNode, width: number, height: number): { x: number, y: number } {
  ...
  return {
    ...
    forceY: (node: D3DAGNode) => {
      ...
      if (node.layer === 2) {
        ...
        const specialists = ['cardiology', 'neurology', 'pulmonology', 'emergency'];
...
export const ZOOM_CONFIG = {
  ...
  translateExtent: [[-500, -500], [1700, 1300]] as [[number, number], [number, number]],
  duration: 300,
  wheelDelta: -0.001,
  touchDelta: 0.01
};
...
export function getParticleDirection(link: D3DAGLink): 'forward' | 'reverse' | 'both' {
