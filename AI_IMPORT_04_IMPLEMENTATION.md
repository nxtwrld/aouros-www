...
export const prescriptionExtractor = traceNode("prescription_extractor")(async (
  state: DocumentProcessingState,
): Promise<Partial<DocumentProcessingState>> => {
...
export function medicalRouter(state: DocumentProcessingState): string {
  ...
  if (errors && errors.length > 0) {
    const hasClassificationError = errors.some(
      (error) => error.node === "medical_classifier",
    );
  ...
  selectOptimalProvider(criteria: SelectionCriteria): AIProvider {
    const candidates = Array.from(this.providers.values()).filter(
      (provider) => {
        // Check requirements
        if (
          criteria.requirements.supportsVision &&
          !provider.capabilities.supportsVision
        ) {
          return false;
        }
        if (
          criteria.requirements.supportsStructuredOutput &&
          !provider.capabilities.supportsStructuredOutput
        ) {
          return false;
        }
        return true;
...
export async function executeWithFallback<T>(
  ...
  try {
    ...
    if (fallbackProvider) {
      console.warn(
        `Provider ${providerName} failed, falling back to ${fallbackProvider.name}`,
      );
